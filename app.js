...
fileInput.addEventListener("change", async (event) => {
  ...
  try {
    ...
    if (isHeicFile) {
      showStatus(
        "✅ HEIC/HEIF file detected! Converting to JPEG...",
        "loading",
      );
      ...
      const convertedImg = createImageElement(
        convertedUrl,
      ...
      const compressionRatio = (
        ((file.size - convertedBlob.size) / file.size) *
        100
      ).toFixed(1);
      ...
      showStatus(
        "ℹ️ This is not a HEIC/HEIF file. Displaying as regular image.",
        "info",
      );
